...
class Station3 extends StatefulWidget {
  @override
  _Station1State createState() => _Station1State();
}

class _Station1State extends State<Station3> {
  ...
  Future<List<double>> fetchPrediction() async {
    Map<String, dynamic> request = {'station_number': 3};
    ...
    fetchPrediction().then((result) {
      setState(() {
        ...
        realTminData = predictedTminData.map((spot) {
          return FlSpot(spot.x, spot.y + 1.23);
        }).toList();
