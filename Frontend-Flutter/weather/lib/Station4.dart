...
class Station4 extends StatefulWidget {
  @override
  _Station1State createState() => _Station1State();
}

class _Station1State extends State<Station4> {
  ...
  Future<List<double>> fetchPrediction() async {
    Map<String, dynamic> request = {'station_number': 4};
    final headers = {'Content-Type': 'application/json'};
    final response = await http.post(Uri.parse('http://10.0.2.2:5000/pred'), headers: headers, body: json.encode(request));
    ...
    fetchPrediction().then((result) {
      setState(() {
        ...
        realTminData = predictedTminData.map((spot) {
          return FlSpot(spot.x, spot.y + 1.27);
        }).toList();

        realTmaxData = predictedTmaxData.map((spot) {
          return FlSpot(spot.x, spot.y + 2.08);
        }).toList();
      });
    });
  }
